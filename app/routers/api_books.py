...
async def update_book(response: Response, update_book:sche_books.BookCreate, db: Session = Depends(get_db)):
	try:
...
async def delete_book(response: Response, book_title: str, db: Session = Depends(get_db)):
	try:
